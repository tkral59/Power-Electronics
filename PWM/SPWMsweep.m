...
%get_param('HW2SPWM/Three-Phase Series RLC Branch', 'DialogParameters')
...
for i = 1:lenVin
    for j = 1:lenRout
        for k = 1:lenLout
            ...
            %Extract Results
            THDI = simOut.get('Iwaveform').signals.values(end, :) * 100;
            THDV = simOut.get('Vwaveform').signals.values(end, :) * 100;
